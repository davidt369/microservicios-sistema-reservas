{{
  config(
    materialized='table',
    indexes=[
      {'columns': ['anio', 'mes', 'grupo_enfermedad']},
      {'columns': ['departamento', 'codigo_cie10']},
      {'columns': ['es_transmisible', 'anio']}
    ],
    tags=['molap', 'epidemiologico', 'cubo_precalculado']
  )
}}
